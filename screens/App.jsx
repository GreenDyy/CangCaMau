        ...
        <Tab.Navigator initialRouteName='TabHome' screenOptions={{ headerShown: false }}>
            <Tab.Screen name="TabHome" component={HomeScreen} options={{
                tabBarIcon: ({ focused }) => (<Image source={icons.home}
                    style={{ tintColor: focused ? '#00CC00' : 'black', height: 25, width: 25 }} />),
                tabBarLabel: ({ focused }) => (
                    <Text style={{ color: '#005F94', fontSize: 12, fontWeight: 'bold' }}>{focused? 'Trang chủ': ''}</Text>
                )
            }} />
            <Tab.Screen name="OTP" component={OTPScreen} options={{
                tabBarIcon: ({ focused }) => (<Image source={icons.notification}
                    style={{ tintColor: focused ? '#00CC00' : 'black', height: 25, width: 25 }} />),
                tabBarLabel: ({ focused }) => (
                    <Text style={{ color: '#005F94', fontSize: 12, fontWeight: 'bold' }}>{focused? 'Thông báo': ''}</Text>
                )
            }} />
            <Tab.Screen name="OTP1" component={OTPForgetPassScreen} options={{
                tabBarIcon: ({ focused }) => (<Image source={icons.profile} 
                    style={{ tintColor: focused ? '#00CC00' : 'black', height: 25, width: 25 }} />),
                tabBarLabel: ({ focused }) => (
                    <Text style={{ color: '#005F94', fontSize: 12, fontWeight: 'bold' }}>{focused? 'Cá nhân': ''}</Text>
                )
            }} />
        </Tab.Navigator>
    )
